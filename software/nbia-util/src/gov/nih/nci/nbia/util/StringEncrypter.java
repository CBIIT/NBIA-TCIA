...
public class StringEncrypter {
	...
	public String encryptString(String cleartext, String key) throws Exception {
		if (StringUtil.isEmptyTrim(cleartext)) {
			throw new IllegalArgumentException("cleartext string was null or empty");
		}
		return toHex(encryptString(cleartext.getBytes(), key));
	}
	...
	private byte[] getRawKey(String key) throws Exception {
	...
	private byte[] encryptString(byte[] clear, String key) throws Exception {
		SecretKeySpec skeySpec = new SecretKeySpec(getRawKey(key), ALGO);
		Cipher cipher = Cipher.getInstance(ALGO);
		cipher.init(Cipher.ENCRYPT_MODE, skeySpec);
		byte[] encrypted = cipher.doFinal(clear);
		return encrypted;
