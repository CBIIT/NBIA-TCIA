...
@Component( {
    selector: 'nbia-pie-chart-collections',
    templateUrl: './pie-chart-collections.component.html',
    styleUrls: ['../../summary.component.scss']
} )


export class PieChartCollectionsComponent implements OnInit, OnDestroy{
    parentNativeElement: any;

    chartName = 'Collections';
    chartFooter = 'Collection';
    chartFooterPlural = 'Collections';
    showCollectionChart = true;
     ...
     * When the mouse is off the chart - Number of Collections in the chart.<br>
     * When mouse is on the chart - Then name of the Collections under the mouse, and the number of them in the Search results, or total if there are no search results.
     */
    footerText;
    ...
    /**
     * This is connected to and creates our svg element in the DOM with the ID "pie-collections"  this.svg = d3.select( '#pie-collections' ).
     */
    svg;
    ...
    chartType = Consts.COLLECTION;
    showCollectionsDescription = false;
    ...
    constructor( private element: ElementRef, private commonService: CommonService,
                 private apiServerService: ApiServerService, private collectionDescriptionsService: CollectionDescriptionsService,
                 private utilService: UtilService, private pieChartService: PieChartService  ) {
            ...
            data => {
                ...
                if( (!this.utilService.isNullOrUndefined( data )) && (!this.utilService.isNullOrUndefined( data['res'] )) && (data['res'].length > 0) ){
                    for( let crtCount of data['res'] ){
                        if( crtCount.criteria === 'Collections' ){
                            criteriaCounts = crtCount.values;
                        }
                    }
                    this.showCollectionChart = true;
    ...
    buildChart() {
        ...
        if( this.criteriaList.length < 1 ){
            ...
            this.showCollectionChart = false;
            return;
        }
        else{
            this.showCollectionChart = true;
        ...
        // Add an svg to the DOM at svg tag with the ID 'pie-collections'
        this.svg = d3.select( '#pie-collections' )
            .append( 'svg' )
...
;
        this.g = this.svg.selectAll( 'arc' )
            ...
            .on( 'click', ( data ) => {
                this.showCollectionsDescription = true;
                this.descriptionPopupText = this.collectionDescriptionsService.getCollectionDescription( data['data']['criteria'] );
            ...
            .on( 'mouseover', ( data ) => {
                ...
                d3.selectAll( <string>('#path-collections' + data['index']) )
                    .transition()
                    .duration( 200 )
        ...
        this.g.append( 'path' )
            ...
            .attr( 'id', ( data ) => {
                ...
                return 'path-collections' + index.toString();
            } )
