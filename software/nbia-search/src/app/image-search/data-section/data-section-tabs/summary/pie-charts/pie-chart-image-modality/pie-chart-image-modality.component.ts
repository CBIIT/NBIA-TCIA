...
@Component( {
    selector: 'nbia-pie-chart-image-modality',
    templateUrl: './pie-chart-image-modality.component.html',
    styleUrls: ['../../summary.component.scss']
} )

export class PieChartImageModalityComponent implements OnInit, OnDestroy{
    parentNativeElement: any;

    chartName = 'Image Modality';
    chartFooter = 'Image Modality';
    chartFooterPlural = 'Image Modalities';
    showImageModalityChart = true;
     ...
     * When the mouse is off the chart - Number of Image Modalities in the chart.<br>
     * When mouse is on the chart - Then name of the Image Modalities under the mouse, and the number of them in the Search results, or total if there are no search results.
     */
    footerText;
    ...
    /**
     * This is connected to and creates our svg element in the DOM with the ID "pie-image-modality"  this.svg = d3.select( '#pie-image-modality' ).
     */
    svg;
    ...
    chartType = Consts.MODALITY;
    showModalityDescription = false;
    ...
    constructor( private element: ElementRef, private commonService: CommonService,
                 ...
                 private modalityDescriptionsService: ModalityDescriptionsService, private pieChartService: PieChartService ) {
            ...
            data => {
                ...
                if( (!this.utilService.isNullOrUndefined( data )) && (!this.utilService.isNullOrUndefined( data['res'] )) && (data['res'].length > 0) ){
                    for( let crtCount of data['res'] ){
                        if( crtCount.criteria === 'Image Modality' ){
                            criteriaCounts = crtCount.values;
                        }
                    }
                    this.showImageModalityChart = true;
    ...
    buildChart() {
        ...
        if( this.criteriaList.length < 1 ){
            ...
            this.showImageModalityChart = false;
            return;
        }
        else{
            this.showImageModalityChart = true;
        ...
        // Add an svg to the DOM at svg tag with the ID 'pie-image-modality'
        this.svg = d3.select( '#pie-image-modality' )
            .append( 'svg' )
        ...
        this.g = this.svg.selectAll( 'arc' )
            ...
            .on( 'click', ( data ) => {
                this.showModalityDescription = true;
                this.descriptionPopupText = this.modalityDescriptionsService.getModalityDescription( data['data']['criteria'] );
                this.descriptionPopupHeading = data['data']['criteria'] + '\n';
            ...
            .on( 'mouseover', ( data ) => {
                ...
                d3.selectAll( <string>('#path-image-modality' + data['index']) )
                    .transition()
                    .duration( 200 )
        ...
        this.g.append( 'path' )
            ...
            .attr( 'id', ( data ) => {
                ...
                return 'path-image-modality' + index.toString();
            } )
    ...
    /**
     * @TODO this is not used, double check then delete.
     * @param e
     */
