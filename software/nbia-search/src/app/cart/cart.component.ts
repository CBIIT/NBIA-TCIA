        ...
        this.commonService.downloadCartAsCsvEmitter.pipe( takeUntil( this.ngUnsubscribe ) ).subscribe(
            data => {
                ...
                if( Properties.DEBUG_CURL ){
                    let curl = 'curl -O -H \'Authorization:Bearer  ' + this.apiServerService.showToken() + '\' -k \'' + Properties.API_SERVER_URL + '/nbia-api/services/v4/getSeriesMetadata' + '\' -d \'' + query + '\'';
                    console.log( 'downloadCartAsCsv: ', curl );
                }

                let csvDownloadUrl = Properties.API_SERVER_URL + '/nbia-api/services/v4/getSeriesMetadata';
