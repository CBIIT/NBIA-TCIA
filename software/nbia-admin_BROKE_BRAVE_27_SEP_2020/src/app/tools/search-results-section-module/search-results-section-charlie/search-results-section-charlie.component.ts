...
@Component({
  selector: 'nbia-search-results-section-charlie',
  templateUrl: './search-results-section-charlie.component.html',
  styleUrls: ['./search-results-section-charlie.component.scss']
})
export class SearchResultsSectionCharlieComponent implements OnInit {
    searchResultsSelectedCount = 0;
    searchResults = [];
    currentCineModeBravoSeriesIndex = -1;
    ...
    constructor( private apiService: ApiService, private cineModeBravoService: CineModeBravoService,
    ...
    async ngOnInit() {
        ...
        this.searchResultByIndexService.searchResultsByIndexEmitter.pipe( takeUntil( this.ngUnsubscribe ) ).subscribe(
            data => {
                if( this.currentCineModeBravoSeriesIndex < (this.searchResults.length - 1) ){
                    this.currentCineModeBravoSeriesIndex++;
                    this.cineModeBravoService.openCineMode( this.searchResults[this.currentCineModeBravoSeriesIndex], this.collectionSite, this.currentCineModeBravoSeriesIndex );
                }
            } );

        // closeCineModeBravoEmitter
        this.cineModeBravoService.closeCineModeEmitter.pipe( takeUntil( this.ngUnsubscribe ) ).subscribe(
            () => {
                this.currentCineModeBravoSeriesIndex = -1;
            } );
    ...
    // Cine-mode viewer
    onClickCineModeBravo( i ) {
        this.currentCineModeBravoSeriesIndex = i;

        this.cineModeBravoService.openCineMode( this.searchResults[i], this.collectionSite, i );
    }
