...
@Component( {
    ...
    styleUrls: ['./query-most-recent-submission-date.component.scss', '../left-section/left-section.component.scss']
} )

export class QueryMostRecentSubmissionDateComponent implements OnInit, OnDestroy{
    @Input() isTop = false;
    @Input() currentTool;
    ...
    constructor( private utilService: UtilService, private querySectionService: QuerySectionService,
    ...
    validateDateRange() {
        ...
        // We need to do this because the bound dataModels are not keeping the ['formatted'] field.
        this.toDateModel['formatted'] = this.setFormattedDate( this.toDateModel['date'] );
        this.fromDateModel['formatted'] = this.setFormattedDate( this.fromDateModel['date'] );
