...
import {PgMemberService} from './pgmemberservice';
...
export class PgMemberList {
    ...
    constructor(private pgmemberService: PgMemberService, public ref: DynamicDialogRef, public config: DynamicDialogConfig) { }

    ngOnInit() {
//		console.log("pg name=" + this.config.data.id);
		this.pgmemberService.getPgUsers(this.config.data.id).subscribe((users:string[]) =>{
		this.users = users;} , 
		error => {
