...
export class AppComponent implements OnInit {
  ...
  constructor(
    ...
    this.globals.serviceUrl =
      window.location.protocol +
      "//" + window.location.host + "/nbia-api/services/v3/";
    ...
    this.appservice.getSerVerHostName().subscribe(
      (data) => {
    ...
    this.appservice.getConfigParams().subscribe(
      (config: Config[]) => {
        this.config = config;
        ...
        console.log(
          "show ug = " +
            this.config[2].paramValue.toLowerCase() +
            "  show pg=" +
            this.config[3].paramValue.toLowerCase()
        );
      },
      (error) => {
        this.handleError(error);
   ...
   // this.commit = this.globals.uatVersionCommit;
	this.version = this.appservice.getSharedConfig('version');
    this.currentYear = this.appservice.getSharedConfig('currentYear');
    this.commit = this.appservice.getSharedConfig('releaseCommit');
  }
...
*/
  ...
  handleChange(e) {
    ...
    if (
      this.tabView.tabs[e.index].header == "Protection Element" &&
      this.peComponent.loadingComplete == false
    )
      this.loadingDisplayService.setLoading(
        true,
        "Loading existing collection and site data..."
      );

    if (
      this.tabView.tabs[e.index].header == "Protection Group" &&
      this.pgComponent.loadingComplete == false
    ) {
      this.loadingDisplayService.setLoading(
        true,
        "Loading protection group data..."
      );
    }

    if (
      this.tabView.tabs[e.index].header == "User Group" &&
      this.groupComponent.loadingComplete == false
    )
