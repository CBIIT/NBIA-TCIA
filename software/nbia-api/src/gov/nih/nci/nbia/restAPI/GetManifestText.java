...
public class GetManifestText extends getData {
	...
	public Response constructResponse(@FormParam("list") List<String> list, @FormParam("password") String password,
			@FormParam("includeAnnotation") String includeAnnotation,
			@FormParam("manifestFileName") String manifestFileName) {
		...
		try {
			...
			// For version 2 download servlet, the user name and password will
			// be authenticated at the back end. So no need for the
			// following lines
