...
@Path("/getExtendedSimpleSearch")
public class GetExtendedSimpleSearch extends getData{
	...
	public Response constructResponse(MultivaluedMap<String, String> inFormParams) {
        ...
        for (PatientSearchResult patient:patients){
        	Set<Integer> patientSeries=patient.computeListOfSeriesIds();
        	for (Integer identifier:patientSeries){
        		seriesList.add(identifier.toString());
        	}
        	
        }
        ec.setSeriesIds(seriesList);
        
        ValueAndCountDAO valueAndCountDAO = (ValueAndCountDAO)SpringApplicationContext.getBean("ValueAndCountDAO");
        Map<String,ExtendedPatientSearchResult> searchResult=valueAndCountDAO.extendedQuery(ec);
        List<ExtendedPatientSearchResult> finalList=new ArrayList<ExtendedPatientSearchResult>();
        for (PatientSearchResult patient:patients){
        	ExtendedPatientSearchResult exsr = searchResult.get(patient.getId().toString());
        	exsr.fromSearchResult(patient);
        	finalList.add(exsr);
        }
        
		return Response.ok(JSONUtil.getJSONforExtendedPatients(finalList)).type("application/json")
				.build();
		} catch (Exception e) {
